...
form.addEventListener('submit', function (event) {
  // Clearing any existing element /text in result div
  result.innerHTML = '';
  // Converting the input value to integer
  const submit = parseInt(amount.value);
  // generating a random index for fall back
  const random = Math.floor(Math.random() * text.length);
  // Checking if the input value is empty or out of range
  if (isNaN(submit) || submit < 0 || submit > 9) {
    // inserting that random index
    result.innerHTML = `<p class="result">${text[random]}</p>`;
  } else {
    // running the for loop input times
